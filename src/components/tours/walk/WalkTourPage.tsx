...
/* ------------------------------------------------------------------ */
/*  One walking tour, as a page.                                       */
/*                                                                     */
/*  This was the body of /tours/hyde-park-walk until Harlem arrived.   */
/*  Both routes are now thin wrappers around it, which is the only     */
/*  way to be sure a second city gets the same page rather than a      */
/*  copy that slowly drifts. Everything city-specific comes off the    */
/*  bundle in src/lib/tours/registry.ts.                               */
/*                                                                     */
/*  The App Store listing promises the narration is free on the site,  */
/*  so keep these routes reachable.                                    */
/* ------------------------------------------------------------------ */
...
export default function WalkTourPage({ bundle }: { bundle: WalkTourBundle }) {
  const { tour, intro, geometry, map, page, mediaDir } = bundle;
  const mainCount = tour.stops.filter((s) => !s.optional).length;
  const detourCount = tour.stops.filter((s) => s.optional).length;

  const stopsLabel =
    detourCount > 0
      ? `The ${spell(mainCount)} stops and ${spell(detourCount)} ${
          detourCount === 1 ? "detour" : "detours"
        }`
      : `The ${spell(mainCount)} stops`;

  const thumb = (src: string) =>
    src.replace(`${mediaDir}/`, `${mediaDir}/thumbs/`);

  return (
    <div className="min-h-screen bg-cream">
      {/* Opener */}
      <section className="relative overflow-hidden border-b border-border bg-cream pb-10 pt-20 md:pb-20 md:pt-28">
        {/* the period document washed into the paper */}
        ...
        <div className="relative mx-auto max-w-6xl px-6">
          ...
          <p className="mt-6 font-display text-lg italic text-ink/65">
            {`${tour.distanceMiles} miles, ${page.terrain}`}
          </p>
      ...
      {/* Why this walk, in full, before the first stop. */}
      <section
        id="why-this-tour"
      ...
      <section
        aria-label={stopsLabel}
        className="border-b border-border bg-[#FBF8F2] py-8 md:py-10"
      >
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
            <h2 className="walk-title text-xl font-semibold text-forest">
              The stops
            </h2>
          ...
          <ol className="-mx-6 mt-5 flex snap-x snap-mandatory gap-3 overflow-x-auto px-6 pb-2 md:mx-0 md:grid md:grid-cols-6 md:gap-3.5 md:overflow-visible md:px-0 md:pb-0">
            ...
            <li className="shrink-0 snap-start">
              <a
                href="#why-this-tour"
                aria-label={`Start with ${intro.title}, read before stop one`}
              ...
              <li key={s.id} className="shrink-0 snap-start">
                ...
                >
                  <span className="walk-plate-flush block rounded-[2px] p-1 shadow-[3px_3px_0_0_rgba(27,58,45,0.08)] transition-transform group-hover:translate-x-[1px] group-hover:translate-y-[1px] group-hover:shadow-[2px_2px_0_0_rgba(27,58,45,0.1)] group-active:translate-x-[2px] group-active:translate-y-[2px] motion-reduce:transition-none">
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={thumb((s.nowImage ?? s.images[0]).src)}
                      alt=""
                      loading="lazy"
      ...
      <section id="tour" className="scroll-mt-16">
        <WalkExperience tour={tour} geometry={geometry} map={map} />
      </section>
              ...
              <div key={item.title} className="walk-plate rounded-[3px] p-6">
                <div className="flex items-center gap-3">
                  <span aria-hidden="true" className="text-rust">
                    ...
                    {i === 3 && (
                      <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
                        <path d="M8 2.6v10.8M5 5.4v5.2M11 5.4v5.2M2.4 7.2v1.6M13.6 7.2v1.6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                      </svg>
                    )}
                  </span>
      ...
      <section className="border-t border-border bg-cream-dark py-14 md:py-20">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <h2 className="walk-title text-2xl font-semibold text-forest md:text-3xl">
            This walk is better on your phone
          </h2>
          <p className="mx-auto mt-4 max-w-[52ch] font-body text-base leading-relaxed text-ink/75">
            The same tour is in our free iPhone app, where the map follows you
            down the street and the whole thing keeps working with no signal.
          </p>
              ...
              {page.related.links.map((link) =>
                link.external ? (
                  ...
                  <Link
                    ...
                    className={
                      link.primary
                        ? "inline-flex items-center rounded-[3px] bg-rust px-8 py-3.5 font-body text-base font-semibold text-white shadow-[4px_4px_0_0_rgba(0,0,0,0.25)] transition-all hover:-translate-y-0.5 hover:bg-rust-dark motion-reduce:transition-none"
                        : "inline-flex items-center rounded-[3px] border border-cream/50 px-8 py-3.5 font-body text-base font-semibold text-cream transition-all hover:-translate-y-0.5 hover:border-cream hover:bg-cream/10 motion-reduce:transition-none"
                    }
                  >
                    {link.label}
                  </Link>
