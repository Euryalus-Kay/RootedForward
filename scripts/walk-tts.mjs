...
const TONE_SETS = {
  ...
  "west-harlem": {
    1: "The welcome, outside a gate the listener cannot go through. Settled and glad they came, a guide who knows the campus from the sidewalk. Plain about the ID rule, no grievance in it. Enjoy the 1897 guide's confidence about raising the ground so the buildings would show above the street. Then quiet and close for Wells; this is one student and one room. Rucker's threat is reported speech, flat. The cross burning is read plainly, no drama, and 'He stayed in his room' stands alone. Warm for the 150 signatures and the 61 letters. The race question on the 1925 application is a record being read out, level, and the last sentence of that paragraph is the point of the stop. End easy, pointed toward the park.",
    2: "Start with the ground, the drop and the rock, so the listener looks down at the pond before they know what it is. Then read the two appraisal forms like a clerk reading a file, the same words for both, and let 'it was red' close the paragraph without any lift. Trace the 1967 drawing with a finger, top door and bottom door, and say Gym Crow the way the protesters said it, without a sneer. Real respect for Suki Terada Ports and Christiane Collins and the West Harlem Community Organization, who were there first. Steady on the Hamilton Hall split; that is a decision being explained, not a quarrel. The bust is told in order, early hours, tunnels, 712 arrests, no rush. Let 'The pond is on the site of the gym that residents and students stopped' land and stop.",
    3: "Scale first, the tallest thing on the ridge, then the church's founding idea read straight. The corporation paragraph is the hinge of the walk, so the pamphlet's one sentence is read slowly as a quotation, and 'Both were true' is level, not a gotcha. Real weight for King, unhurried, and the paragraph about the condemnation is a fact of record. Forman arrives as an interruption, so let the pace change; the manifesto's demands are read straight, including the sentence about force, without softening or relish. The organ playing over him is described, not dramatised. End on the disagreement that remained, quiet, then easy for the directions.",
