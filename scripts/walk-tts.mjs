...
const TOURS = {
  ...
  "west-harlem": {
    1: "The welcome, outside a gate the listener cannot go through. Settled and glad they came. Plain about the ID rule, no grievance in it. Enjoy the 1897 guide's confidence about raising the ground so the buildings would show above the street, and let 'designed from the start to be looked up at' sit for a beat. The last paragraph is a friend telling you what the walk is about, easy and direct.",
    2: "Quiet and close. This is one student and one room. Read the dates plainly. Rucker's threat is reported speech, flat. The cross burning gets no drama at all, and 'He stayed in his room' stands alone. Warm a little for the 150 residents and the letters. Then level for the 1925 form, and read the last sentence about the form sorting people slowly.",
    3: "A week told in order, so keep the clock audible: twenty to one, two o'clock, half past five, the early hours of the thirtieth. Momentum but no excitement. The split between the Black and white students is a decision being explained, not a quarrel. The tunnels and the eighty-six arrests are read evenly, and 712 lands as a number, not a punchline.",
    4: "Start with the view and let the listener look. Then read the two appraisal forms like a clerk reading a file, word by word, the same tone for B and for D, because the gap between them is the point. No indignation. The paragraph about what the map did to a family below the cliff is slow and exact. End by pointing south along the drive.",
    5: "Start with the pond, so they look down before they know what it is. Trace the drawing with a finger, top door and bottom door. Say Gym Crow the way the protesters said it, plainly. Real respect for Suki Terada Ports, Christiane Collins and the West Harlem Community Organization, who were there first. Let 'the gym that residents and students stopped' close the stop.",
    6: "Look around first, naming the buildings like a neighbor. Then the corporation's own sentence about separating the Heights from Harlem is read exactly and without comment. Munnecke's advice is the hinge of the whole project, so slow down and read the two sentences about the terms doing the sorting. 'Horizontal restrictions' is a quotation; give it that weight. The Chicago link is a plain aside.",
    7: "Scale first, then the two facts about the church side by side, evenly. Real weight for King, unhurried, and the condemnation is a fact of record. Forman arrives as an interruption, so let the pace change; the manifesto's demands are read straight, including the sentence about force. The organ and the exit are described, not performed. End on who controlled the money, quietly.",
    8: "The most careful stop. Even and exact throughout. The 1,626 families are read slowly. The two neighbors disagreeing in the Guardian get equal weight. Read the cooperative figures like a clerk, 750, 21, 3,000, 84, each number clean. The opening percentages and the institutions' third are a record being read out. Warm at last for Antonia Pantoja in her living room, and keep the medal simple.",
    9: "Have the listener turn around, then compare the two sides plainly. The public housing officials' sentence is read as their words. The Eggers and Higgins connection is delivered flat and allowed to land. Careful and honest on the relocation figure we are not using, and why. Real warmth for Sarah Martin at the end, and an easy pointer to the detour.",
    10: "The detour, so a little brighter, a guide glad you came the extra blocks. The five demands are read as a list, one clean beat each, no rush. The chained gates and the two weeks are told with momentum. Genuine pleasure at what changed by that fall, and slow for 'After the strike it did it in 1970.' The last two sentences about the separate door and the gates are the walk's argument; read them plainly and stop.",
    11: "Under the elevated, so the voice can be closer to the street and lighter. Enjoy the milk plant, the horses and the doomed headline. Then quiet and steady for 1949 and the university's first step down the hill. The list of what the valley was is read as a list of ordinary things. End with an easy pointer north.",
    12: "A planner's evenness for the two plans, the university's and the neighborhood's, side by side. The court case is a decision being reported; the plate has the detail, so the narration stays plain. The dollar figures in the agreement are read cleanly, and the caution after them is part of the same breath. Warm and slow for Hilda Muentes and Luisa Henriquez, and let 'sixteen blocks north' sit.",
    13: "Look up first and let them see the arch. Read Abbott's caption like a caption, item by item. The list of what the valley was, milk, meat, storage, a ferry, a gas station, is read slowly. Steady on the holdouts. The last two sentences about what the court did and did not decide are the point; read them level and then send the listener to the river.",
    14: "The finale, on the water, and after thirteen stops of institutions this one belongs to residents. Open easy. The hotel and the motel are named plainly and a little drily. Real pleasure that Sarah Martin and Joan Levine turn up here. Gentle on the map, the paths and the sculptures. Read the closing sentence about a decision and an answer slowly, with a beat before it, and end warm on the thanks.",
  },
  "jackson-park": {
    data: "src/lib/tours/jackson-park-walk.ts",
    out: "public/media/jackson-park-walk/audio",
